...
setwd("/Users/hojin/Dropbox/project/enrichmate/20241209_final/")

# 1. Setting environment for analysis ------
## get database ----
gobp <- gmtPathways("m5.go.bp.v2024.1.Mm.symbols.gmt")
gobp_hj <- read.table("20241213_m5.go.bp.v2024.1.Mm.term.id_mapping_hj.txt",header = T, sep = "\t") # this file is made from json file

gobp_ancestor <- as.list(GOBPANCESTOR)
names(gobp) %>% length() # 7713
which(gobp_hj$GOID %in% names(gobp_ancestor)) %>% length() # 7704
...
#    df <- rbind(df, df_tmp)
# }
#saveRDS(df, file = "20241213_GOBP_ANCESTOR_mouse_df_hj.rds")

## run here ----
df <- readRDS("20241213_GOBP_ANCESTOR_mouse_df_hj.rds")
...
gobp <- gobp[gobp_hj$GOTERM] # 7704 GOBPs

# 2. Get GOBP results ------
# in this part, please load your gsea results in mydf
mydf <- readRDS("/Users/hojin/Downloads/fgseaResTidy.rds")
#mydf <- read.table("20240122_cr_base_y2_gsea_results_hj.txt", header = T, sep = "\t")
mydf <- mydf %>% dplyr::filter(padj < 0.05) %>% dplyr::filter(NES > 0)
...
setdiff(mydf$pathway, gobp_hj$GOTERM) %>% length()

## compute IC score ------
library(GOSemSim)
library(simplifyEnrichment)
if (!("MmGO" %in% ls())) {
  set.seed(1234); MmGO <- godata('org.Mm.eg.db', ont = "BP", computeIC = T) # make IC score using GOsemsim package 
}

sim_method <- "Rel"
set.seed(1234); w <- GOSemSim::mgoSim(goids, goids, semData = MmGO, measure = sim_method, combine = NULL) # w is similarity matrix. 
na_goid <- names(which(is.na(w[1,]))) # select NA goid.
goids <- goids[which(!goids %in% na_goid)]
set.seed(1234); w <- GOSemSim::mgoSim(goids, goids, semData = MmGO, measure = sim_method, combine = NULL) # w is similarity
...
test_range = seq(1,10) # if you want to increase or decrease the number of clusters to be tested, change this range. 
